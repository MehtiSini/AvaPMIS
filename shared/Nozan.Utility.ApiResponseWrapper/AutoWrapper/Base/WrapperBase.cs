...
using AvaPMIS.Gateway.AutoWrapper.Filters;
...
namespace AvaPMIS.Gateway.AutoWrapper.Base
{
...
#pragma warning restore IDE1006 // 命名样式
        public WrapperBase(RequestDelegate next,
                          IOptions<AutoWrapperOptions> options,
        ...
        {
            if (awm.IsSwagger(context, _options.SwaggerPath)
                || awm.IsOpenIdWellKnown(context, _options.OpenIDWellKnownPath)
                || awm.IsOpenId(context, _options.OpenIDPath)
                || !awm.IsApi(context) ||
                awm.IsExclude(context, _options.ExcludePaths)
                ||(_options.IgnoreStartupPath&&awm.IsStartupPath(context)))
            {
                await _next(context);
            }
            else
