...
using AvaPMIS.Gateway.AutoWrapper.Extensions;
using AvaPMIS.Gateway.AutoWrapper.Helpers;
...
using StringExtension = AvaPMIS.Gateway.AutoWrapper.Extensions.StringExtension;
using TypeConverterExtension = AvaPMIS.Gateway.AutoWrapper.Extensions.TypeConverterExtension;
...
namespace AvaPMIS.Gateway.AutoWrapper
{
    internal class AutoWrapperMembers
    {
        ...
        public AutoWrapperMembers(IOptions<AutoWrapperOptions> options,
        ...
        public async Task<string> ReadResponseBodyStreamAsync(Stream bodyStream)
        {
            ...
            var (IsEncoded, ParsedText) = StringExtension.VerifyBodyContent(responseBody);
        ...
        {
            var (IsEncoded, ParsedText) = StringExtension.VerifyBodyContent(body.ToString());
        ...
        public async Task HandleSuccessfulRequestAsync(HttpContext context, object body, int httpStatusCode)
        {
            ...
            {
                Wrappers.ApiResponse apiResponse = new Wrappers.ApiResponse();
                ...
                else
                {
                    ...
                    else if (bodyText.Contains(nameof(Wrappers.ApiResponse.Result)))
                        apiResponse = JsonConvert.DeserializeObject<Wrappers.ApiResponse>(bodyText);
                    else
                        apiResponse = new Wrappers.ApiResponse();
                }
        ...
        public bool IsOpenIdWellKnown(HttpContext context, string openIdWellKnownPath)
        {
            return context.Request.Path.StartsWithSegments(new PathString(openIdWellKnownPath));
        }
        public bool IsOpenId(HttpContext context, string openIdPath)
        {
            return context.Request.Path.StartsWithSegments(new PathString(openIdPath));
        }
        public bool IsStartupPath(HttpContext context)
        {
            return context.Request.Path==""|| context.Request.Path=="/";
        }
        ...
        {
            var apiResponse = new Wrappers.ApiResponse($"{httpMethod} {ResponseMessage.Success}", content, !_options.ShowStatusCode ? 0 : httpStatusCode, GetApiVersion());
        ...
        private Wrappers.ApiResponse GetErrorResponse(int httpStatusCode, object apiError)
            => new Wrappers.ApiResponse(!_options.ShowStatusCode ? 0 : httpStatusCode, apiError) { Version = GetApiVersion() };

        private Wrappers.ApiResponse GetSucessResponse(Wrappers.ApiResponse apiResponse, string httpMethod)
        {
        ...
        private (bool, object) ValidateSingleValueType(object value)
        {
            ...
            if (TypeConverterExtension.IsWholeNumber(result)) { return (true, TypeConverterExtension.ToInt64(result)); }
            if (TypeConverterExtension.IsDecimalNumber(result)) { return (true, TypeConverterExtension.ToDecimal(result)); }
            if (TypeConverterExtension.IsBoolean(result)) { return (true, TypeConverterExtension.ToBoolean(result)); }
