...
namespace Nozhan.Abp.Utilities.Extensions {
    public static class TypeHelper {
        ...
        public static Type ResolveType(string fullTypeName, Type expectedBase = null) {
            ...
            if (type == null) {
                throw new Exception(string.Format("Unable to resolve type: \"{0}\".", fullTypeName));
                return null;
            }

            if (expectedBase != null && !expectedBase.IsAssignableFrom(type)) {
                throw new Exception(string.Format("Type \"{0}\" must be assignable to type: \"{1}\".", fullTypeName, expectedBase.FullName));
                return null;
            }
