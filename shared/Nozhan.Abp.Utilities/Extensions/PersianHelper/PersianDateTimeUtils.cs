...
namespace System
{
    ...
    public static class PersianDateTimeUtils
    {
        ...
        public static DateTime? ToGregorianDateTime(this string persianDateTime)
        {
            ...
            {
                var y = rawDate.Substring(0, 4);
                var m = rawDate.Substring(4, 2);
                var d = rawDate.Substring(6, 2);
                var stStr = $"{y}/{m}/{d}" + (!string.IsNullOrEmpty(rawTime)? " " + rawTime : string.Empty);
                return stStr.ToGregorianDateTime();
            }
