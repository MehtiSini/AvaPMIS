...
using AvaPMIS.PublicGateway.ApiResponseWrapper.Extensions;
...
{
    public class ApiResponseWrapperMiddleware : OcelotMiddleware
    {
...
#nullable disable
        ...
        public ApiResponseWrapperMiddleware(
          ...
          RequestDelegate next,
            ...
            : base(loggerFactory.CreateLogger<ApiResponseWrapperMiddleware>())
        {
        ...
        public async Task Invoke(HttpContext httpContext, ILogger<ApiResponseWrapperMiddleware> logger)
        {
            ...
            _logger.LogWarning("response wrapper called!");
            try
            {
                ...
                using (var responseBody = new MemoryStream())
                {
                    ...
                    try
                    {
                        ...
                        _logger.LogWarning("respnse recieved!");
                        DownstreamResponse downStreamresponse = httpContext.Items.DownstreamResponse();
                        
                        string responseBodyContent = null;
                        if (downStreamresponse != null)
                        {
                            responseBodyContent = await FormatResponse(downStreamresponse);
                        }
                        else
                        {
                            responseBodyContent = await FormatResponse(response);
                        }
                        _logger.LogWarning(responseBodyContent);
                        var statusCode = downStreamresponse != null ? downStreamresponse.StatusCode : (HttpStatusCode)httpContext.Response.StatusCode;
                        if (new HttpResponseMessage(statusCode).IsSuccessStatusCode)
                        {
                            ...
                            _logger.LogWarning("after handle success!");
                        }
                        else
                        {
        ...
        private async Task HandleExceptionAsync(HttpContext httpContext, Exception exception)
        {
            ...
            DataModel.ApiResponse apiResponse = null;
            var code = 0;
            if (exception is FluentValidation.ValidationException)
            {
                var ex = exception as FluentValidation.ValidationException;
                apiError = new ApiError(ResponseMessageEnum.ValidationError.GetDescription(), ex.Errors.ToValidationErrors())
                {
                    IsError = true,
                    ValidationErrors = ex.Errors.ToValidationErrors(),
                    ReferenceErrorCode = Microsoft.IdentityModel.Tokens.CollectionUtilities.IsNullOrEmpty(ex.Errors) ? "" : ex.Errors.ToList()[0].ErrorCode,
                    ReferenceDocumentLink = ex.HelpLink
                };
            ...
            apiResponse = new DataModel.ApiResponse(code, ResponseMessageEnum.Exception.GetDescription(), null, apiError);
        ...
        private async Task HandleNotSuccessRequestAsync(HttpContext httpContext, string errorMessage, int code)
        {
            ...
            var apiResponse = new DataModel.ApiResponse(code, apiError);
        ...
        private Task HandleSuccessRequestAsync(HttpContext httpContext, object body, int code)
        {
            ...
            if ((type.Equals(typeof(Newtonsoft.Json.Linq.JObject)) || type.Equals(typeof(Newtonsoft.Json.Linq.JArray))) && IsApiResponse(bodyContent))
            {
                ...
                apiResponse = JsonConvert.DeserializeObject<DataModel.ApiResponse>(bodyText);
                ...
                {
                    apiResponse = new DataModel.ApiResponse(code, ResponseMessageEnum.Success.GetDescription(), bodyContent, null);
                    jsonString = JsonConvert.SerializeObject(apiResponse);
                }
            }
            else
        ...
        private bool IsApiResponse(object reponse)
        {
            ...
            var apiResponseProperties = new DataModel.ApiResponse(200, "").GetPublicProperties().Select(p => p.Name);
        ...
        private async Task<string> FormatResponse(DownstreamResponse response)
        {
            return await response.Content.ReadAsStringAsync();
        }
        private string ConvertToJSONString(int code, object content)
        {
            return JsonConvert.SerializeObject(new DataModel.ApiResponse(code, ResponseMessageEnum.Success.GetDescription(), content, null, string.Empty), JSONSettings());
        }
        private string ConvertToJSONString(DataModel.ApiResponse apiResponse)
        {
