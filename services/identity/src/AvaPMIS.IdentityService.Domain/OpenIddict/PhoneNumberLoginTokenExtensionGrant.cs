    ...
    public class PhoneNumberLoginTokenExtensionGrant : ITokenExtensionGrant, ITransientDependency
    {
        ...
        public virtual async Task<IActionResult> HandleAsync(ExtensionGrantContext context)
        {
            ...
            var localizer = context.HttpContext.RequestServices
                .GetRequiredService<IStringLocalizer<IdentityServiceResource>>();
            ...
            var identityUser =
                await uniquePhoneNumberIdentityUserRepository.FindByPhoneNumberAsync(phoneNumber,null,false);

            if (password.IsNullOrWhiteSpace())
            {
                var result = code != null && await identityUserManager.VerifyUserTokenAsync(identityUser,
                ...
                {
                    //if phone number is not confirmed => now we confirm it 
